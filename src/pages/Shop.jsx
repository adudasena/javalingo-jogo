...
export default function Shop() {
  ...
  function buy(id, price) {
    ...
    if (s.coins < price) {
      return alert('Moedas insuficientes. Jogue para ganhar mais!')
    }
            ...
            <div className="card" key={sk.id}>
              ...
              {s.ownedSkins.includes(sk.id) ? (
                <button
                  ...
                  disabled={s.activeSkin === sk.id}
                >
                  {s.activeSkin === sk.id ? 'Ativo' : 'Ativar'}
                </button>
              ) : (
                <button className="btn" onClick={() => buy(sk.id, sk.price)}>
                  Comprar
                </button>
              )}
            </div>
